...
class MyHTMLParser(HTMLParser):
    ...
    def handle_data(self, data):
        ...
        # get quote
        if -1 != string.find(str(starttag_text), "yfs_l84_%s" % ticker.lower()) and len(data.strip()) > 0:
            sys.stdout.write("\t" + data)
            
        # get percentage changed
        if -1 != string.find(str(starttag_text), "yfs_p20_%s" % ticker.lower()) and len(data.strip()) > 0:
            sys.stdout.write("\t")
            if self.negative:
                sys.stdout.write("-")
        ...
        if -1 != string.find(str(starttag_text), "yfs_g53_%s" % ticker.lower()) and len(data.strip(" -")) > 0:
            sys.stdout.write("\t" + data)
